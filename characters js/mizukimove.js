...
function preload()
{
  mizukiIcon = loadImage("/icons/Sanjana.jpg");
}

function setup() {
  createCanvas(1350, 600);
  
  let babyhoodlink = createA("../flashcards-SV/answersChild.html", 'Babyhood',"blank");
  babyhoodlink.position(1020, 100);

  let teenhoodlink = createA("../flashcards-SV/answersTeen.html", 'Teenhood',"blank");
  teenhoodlink.position(760, 320);

  let adulthoodlink = createA("../flashcards-SV/answersAdult.html", 'Adulthood',"blank");
  adulthoodlink.position(110, 420); 
  
  let oldhoodlink = createA("../flashcards-SV/answersOld.html", 'Oldhood',"blank");
...
noTint();
image(mizukiIcon,mizukiIconProp.x, mizukiIconProp.y, 70,70);
...
function keyPressed() {
  if (keyCode === UP_ARROW)
   { mizukiIconProp.y -= mizukiIconProp.speed ;
   
  } else if (keyCode === DOWN_ARROW) {
    mizukiIconProp.y += mizukiIconProp.speed;
  }
  else if (keyCode === LEFT_ARROW) {
    mizukiIconProp.x -= mizukiIconProp.speed ;
